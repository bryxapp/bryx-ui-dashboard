...
import { PhoneInputObj, TextObjTemp } from '../../../../../utils/types/CanvasInterfaces';
...
const PhoneInput = ({ phoneInputObj, handleDragStart, handleDragEnd, isSelected, onSelect, onTransformEnd, handleDragMove, draggable = true }: PhoneInputProps) => {
    ...
    const tempTextShape = createTempTextKonvaShape(phoneInputObj.content);
        ...
        <React.Fragment>
            ...
            >
                ...
                {phoneInputObj.hasLabel && (
                    <InputLabel labelObj={phoneInputObj.label} x={getXAlignment(phoneInputObj.label)} y={getYAlignment() - 25} />
                )}
                {/* Input Placeholder */}
                <Text
                    x={getXAlignment(phoneInputObj.content)}
                    y={getYAlignment()}
                    text={`${phoneInputObj.content.value}`}
                ...
                <Text
                    ...
                    y={5}
                    text='📞'
                    fontSize={phoneInputObj.content.fontSize * .6}
                    fill={'gray'}
                    scaleX={1}
                    scaleY={1} />
            </Group>
